...
- **Authentication via gateway-issued virtual keys**, not a client-supplied
  identity — a budget can't be evaded by a caller simply omitting or
...
- **Provider fallback that never hands a client a broken partial response**
  — for streaming, fallback is confirmed to work *before* anything is
...
- **The dashboard has no authentication.** Anyone who can reach the port
  can see aggregate spend, per-user breakdowns, and recent request
  metadata. Fine on a private network; not acceptable exposed any wider
