...
Once you've pushed this repo to your own GitHub org and tagged a release,
`go install github.com/<your-org>/ai-cost-guard/cmd/ai-guard@latest` works
too — until then, the module path in `go.mod` doesn't resolve to anything
published.

Prebuilt cross-platform binaries are produced by
[`.github/workflows/release.yml`](.github/workflows/release.yml) on any
`v*` tag, once you're publishing releases. Or use [Docker](#deployment).
...
Provider config keys that aren't `openai`, `anthropic`, `groq`, or
...
that key (there's nothing to cap it against). If you hand-edit
...
of OpenAI, Anthropic, Groq, and Together models (see the file for the exact
...
port can view aggregate spend/usage. Put it behind your own network
boundary or reverse-proxy auth if that's a concern.
...
- **`ai-guard init`** — interactive setup (`internal/cli/init.go`): pick
  ...
  to confirm you meant "unlimited," since it's easy to mistype.
